# Numerical solver for shallow water equation on logically sphere mesh under Torch framework
import torch
...
def fvm_1storder(q0, vert_3d, cell_center, cell_area, dt, r, nhalo):
    """
...
def fvm_2ndorder_space(q0, vert_3d, cell_center, cell_area, dt, r, nhalo):
    """
    Finite volume method for shallow water equation
    q0: double[4, M+nhalo*2, N+nhalo*2]
    vert_3d: double[3, M+1, N+1]
...
def fvm_heun(q0, vert_3d, cell_center, cell_area, dt, r, nhalo):
    """
    ...
    """
    qs = fvm_2ndorder_space(q0, vert_3d, cell_center, cell_area, dt, r, nhalo)
    qss = fvm_2ndorder_space(qs, vert_3d, cell_center, cell_area, dt, r, nhalo)
    q1 = 0.5 * (q0 + uss)
    return q1

def fvm_TVD_RK(q0, vert_3d, cell_center, cell_area, dt, r, nhalo):
    """
    q0: double[3,M+nhalo*2,N+nhalo*2], state matrix of current timestep
    q1: double[3,M+nhalo*2,N+nhalo*2], state matrix of next timestep
    vert: double[M+1,N+1,2], matrix of vertices
    ...
    qs = fvm_2ndorder_space(q0, vert_3d, cell_center, cell_area, dt, r, nhalo)
    #stage2
    qss = (3/4)*q0 + (1/4)*fvm_2ndorder_space(qs, vert_3d, cell_center, cell_area, dt, r, nhalo)
    #stage3
    q1 = (1/3)*q0 + (2/3)*fvm_2ndorder_space(qss, vert_3d, cell_center, cell_area, dt, r, nhalo)
    # print(torch.all(q1[0,:,:]>0))
    return q1

def integrate(q_out, q_init, vert_3d, cell_center, cell_area, r, save_ts, nhalo):
    """
    ...
    while save_it < nT:
        ...
        q0 = fvm(q0, vert_3d, cell_center, cell_area, dt, r, nhalo)
        it += 1
        T += dt
        if it % 10 == 0:
...
def integrate_end(q_out, q_init, vert_3d, cell_center, cell_area, r, nhalo):
    """
    Solve shallow water equation on sphere
    q_init: double[4, M, N], initial condition
    q_out: double[4, M, N]
...
if __name__=="__main__":
    ...
    if args.save_xdmf:
        ...
        integrate(q_out, q_init, vert_3d, cell_center_3d, cell_area, r, save_ts, nhalo)
        end_time = time.time()
        print(f"End computing------------------------\nComputing time:",end_time - start_time,"s")
        save_ts = save_ts.cpu().numpy()
        q_out = q_out.cpu().detach().numpy()
        vert_3d = vert_3d.cpu()
        MeshGrid.save_sphere_grid_xdmf(f"{path}/sphere-SWE-{M}-{N}-{args.flux}.xdmf", zip(save_ts, q_out), vert_3d)

    else:
        q_out = torch.zeros_like(q_init)
        print("Start computing------------------------")
        start_time = time.time()
        integrate_end(q_out, q_init, vert_3d, cell_center_3d, cell_area, r, nhalo)
        end_time = time.time()
        print(f"End computing-------------------------\nComputing time:",end_time - start_time,"s")
        q_out = q_out.cpu()
        vert_3d = vert_3d.cpu()
        if args.save_vtk:
            MeshGrid.save_sphere_grid_vtk(f"{path}/sphere-SWE-{M}-{N}-{args.flux}", q_out, vert)

        if args.plot:
            plt.figure(figsize=(10,8))
            plt.imshow(q_out[0,...], extent=[*x_range, *y_range], cmap='viridis', origin="lower")
            plt.colorbar()
            plt.savefig(f"{path}/sphere-SWE-{M}-{N}-{args.flux}.png")
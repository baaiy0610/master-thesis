# Numerical solver for 2D shallow water equation on rectangular mesh with bed under Torch framework
import torch
...
def generate_perlin_noise_2d(nx, ny, res=[2,2]):
    def f(t):
        return 6*t**5 - 15*t**4 + 10*t**3

    delta = (res[0] / nx, res[1] / ny)
    d = (nx // res[0], ny // res[1])
...
def riverbed(bed_height_l, bed_height_r, h):
    return -g*0.5*h*(bed_height_r-bed_height_l)

def fvm_1storder(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy):
    """
    ...
    bed_height_l = bed_height_halo[nhalo-1:-nhalo-1, nhalo:-nhalo]
    bed_height_r = bed_height_halo[nhalo+1:-nhalo+1, nhalo:-nhalo]
    bed_height_b = bed_height_halo[nhalo:-nhalo, nhalo-1:-nhalo-1]
    bed_height_u = bed_height_halo[nhalo:-nhalo, nhalo+1:-nhalo+1]


    riverbed_sum = torch.zeros_like(flux_sum)
    riverbed_sum[1,...] = riverbed(bed_height_l, bed_height_r, qc[0,...]) / dx
    riverbed_sum[2,...] = riverbed(bed_height_b, bed_height_u, qc[0,...]) / dy

    q1c = qc - dt/cell_area * flux_sum + riverbed_sum * dt
...
def fvm_2ndorder_space(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy):
    """
    Finite volume method for shallow water equation
    q0: double[3, M+nhalo*2, N+nhalo*2]
    vert: double[2, M+1, N+1]
...
def fvm_heun(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy):
    """
    ...
    return: double[3, M+nhalo*2, N+nhalo*2]
    """
    qs = fvm_2ndorder_space(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
    qss = fvm_2ndorder_space(qs, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
    q1 = 0.5 * (q0 + qss)
    return q1

def fvm_TVD_RK(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy):
    """
    q0: double[3, M+nhalo*2,N+nhalo*2], state matrix of current timestep
    q1: double[3, M+nhalo*2,N+nhalo*2], state matrix of next timestep
    vert: double[2, M+1, N+1], matrix of vertices
    ...
    qs = fvm_2ndorder_space(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
    #stage2
    qss = (3/4)*q0 + (1/4)*fvm_2ndorder_space(qs, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
    #stage3
    q1 = (1/3)*q0 + (2/3)*fvm_2ndorder_space(qss, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
    # print(torch.all(q1[0,:,:]>0))
    return q1

def initial_condition(x_range, y_range, M, N, bed_height=None):
    hin, hout = 0.5, 1.0
    xmin, xmax = x_range
    ymin, ymax = y_range
    dx = (xmax - xmin)/M
    ...
    u_init[0, ...] = zz - bed_height
    return u_init

def boundary_condition_reflect(q, nhalo, unsqueeze=True, bedheight=False):
    """
    ...
    if bedheight:
        q = q.unsqueeze(0)
        q = F.pad(q, (nhalo, nhalo, nhalo, nhalo), "reflect").squeeze(0)
...
def boundary_condition_periodic(q, nhalo, unsqueeze=True, bedheight=False):
    """
    Periodic boundary condition
    q: double[3, M+nhalo*2, N+nhalo*2], state matrix with halo of width nhalo
    """
...
def calc_dt(q, vert, cell_area, nhalo):
    ...
    q: double[M+2, N+2, 3], state matrix
    vert: double[M+1, N+1, 2], vertices matrix
...
def integrate(q_out, q_init, vert, cell_area, Te, save_ts, nhalo, bed_height, dx, dy, cal_dt=False):
    """
    Solve shallow water equation on Planar with bed
    ...
    bed_height_halo = boundary_condition(bed_height, nhalo, bedheight=True).squeeze()
    ...
    while save_it < nT:
        if save_ts[save_it] - T < 1e-10:
            q_out[save_it, ...] = torch.cat((q0[:, nhalo:-nhalo, nhalo:-nhalo],
                                               bed_height), dim=0)
            save_it += 1
            if save_it == nT:
                break
        ...
        q0 = fvm(q0, vert, cell_area, dt, nhalo, bed_height_halo, dx, dy)
        it += 1
        T += dt
        if it % 100 == 0:
...
def integrate_end(q_out, q_init, vert, cell_area, Te, nhalo, bed_height, dx, dy, cal_dt=False):
    """
    Solve shallow water equation on Planar with bed
    q_out: double[3, M, N]
    q_init: double[3, M, N], initial condition
    ...
    while T < Te:
        ...
        if it % 100 == 0:
            ...
            total_energy = ((potential + kinetic) * cell_area).sum(dim=(-2, -1)).item()
            print("T =", T, "it =", it, "dt =", dt, "Total energy:", total_energy)
    q_out[...] = torch.cat((q0[:, nhalo:-nhalo, nhalo:-nhalo],
                            bed_height), dim=0)
...
if __name__=="__main__":
    ...
    parser.add_argument("--boundary", default="reflect", type=str)
    ...
    bed_height = generate_perlin_noise_2d(M, N)
    q_init = initial_condition(x_range, y_range, M, N, bed_height)

    vert, cell_area = MeshGrid.regular_grid(x_range, y_range, M, N)
    q_init = q_init.to(device)
    vert = vert.to(device)
    cell_area = cell_area.to(device)
    bed_height = torch.from_numpy(bed_height).unsqueeze(0).to(device)
    ...
    if args.save_xdmf:
        ...
        integrate(q_out, q_init, vert, cell_area, Te, save_ts, nhalo, bed_height, dx, dy)
        end_time = time.time()
        print(f"End computing-------------------------\nComputing time:",end_time - start_time,"s")
        save_ts = save_ts.cpu().numpy()
        q_out = q_out.cpu().numpy()
        vert = vert.cpu()
        MeshGrid.save_quad_grid_xdmf(f"{path}/quad-SWE_bed-{M}-{N}-{args.flux}.xdmf", zip(save_ts, q_out), vert)

    else:
        q_out = torch.zeros((4, M, N), device=device, dtype=torch.double)
        print("Start computing------------------------")
        start_time = time.time()
        integrate_end(q_out, q_init, vert, cell_area, Te, nhalo, bed_height, dx, dy)
        end_time = time.time()
        print(f"End computing-------------------------\nComputing time:",end_time - start_time,"s")
        q_out = q_out.cpu()
        vert = vert.cpu()
        if args.save_vtk:
            MeshGrid.save_quad_grid_vtk(f"{path}/quad-SWE_bed-{M}-{N}-{args.flux}", q_out, vert)
        ...
        if args.plot:
            ...
            #plot 2:
            plt.subplot(1, 2, 2)
            plt.imshow(q_out[3,...], extent=[*x_range, *y_range], cmap='viridis', origin="lower")
            plt.title('River bed')

            plt.show()
            plt.savefig(f"{path}/quad-SWE_bed-{M}-{N}-{args.flux}.png")

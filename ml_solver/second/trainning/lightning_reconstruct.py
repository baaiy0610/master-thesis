...
class LightningCnn(pl.LightningModule):
    ...
    def validation_step(self, test_batch, batch_idx):
        ...
        output = self.forward(x)
        loss = self.cnn_mse_loss(output, y)
        self.log('Val loss', loss*1e9, sync_dist=True)
        #return loss
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        output = self.forward(x)
        loss = self.cnn_mse_loss(output, y)
        self.log('TEST loss', loss*1e9, sync_dist=True)
        #return loss
...
if __name__ == "__main__":
    ...
    test_dataset = TestDataset("./data_test.pt")
    ...
    if args.gpu:
        ...
        if args.retrain:
            trainer = pl.Trainer(accelerator="gpu", devices=-1, auto_select_gpus=True, max_epochs=100, strategy="ddp_find_unused_parameters_false", resume_from_checkpoint="cnn_reconstruct2.ckpt", logger=wandb_logger, check_val_every_n_epoch=1)

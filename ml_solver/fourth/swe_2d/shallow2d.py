# Numerical solver for 2D shallow water equation on logically rectangular mesh on dace framework
import torch
...
def max_eigenvalue(q, all_direction = False):
    ...
    if all_direction:
        res = max(res, torch.sqrt(torch.abs(g*h)) + torch.abs(v))
    return res
...
def fvm_2ndorder_space_classic(q0, vert, cell_area, dt, nhalo):
    """
    Finite volume method for shallow water equation
    q0: double[3, M+nhalo*2, N+nhalo*2]
    vert: double[2, M+1, N+1]
...
def integrate(q_out, q_init, vert, cell_area, Te, save_ts, nhalo):
    """
...
def integrate_end(q_out, q_init, vert, cell_area, Te, nhalo):
    """
    Solve shallow water equation on sphere
    q_out: double[3, M, N]
    q_init: double[3, M, N], initial condition
...
if __name__=="__main__":
    ...
    if args.save_xdmf:
        ...
        q_out = torch.zeros((len(save_ts), 3, M, N), dtype=torch.double, device=device)
        ...
        vert = vert.cpu()
        MeshGrid.save_quad_grid_xdmf(f"{path}/quad-fvm-{M}-{N}-{args.flux}.xdmf", zip(save_ts, q_out), vert)
        ...
        vert = vert.cpu()
        if args.save_vtk:
            MeshGrid.save_quad_grid_vtk(f"{path}/quad-fvm-{M}-{N}-{args.flux}", q_out, vert)

        if args.plot:
            plt.figure(figsize=(10,8))
            plt.imshow(q_out[0,...], extent=[*x_range, *y_range])
            plt.colorbar()
            plt.savefig(f"{path}//quad-fvm-{M}-{N}-{args.flux}-torch.png")

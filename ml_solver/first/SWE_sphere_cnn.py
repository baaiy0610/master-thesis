...
class LightningCnn(pl.LightningModule):
    def __init__(self, mu: torch.Tensor, std: torch.Tensor, mid_channel: int, nstencil_out: int, ninterp: int, stencil_x: int, stencil_y: int):
        ...
        # nn.init.constant_(self.out.weight, 0.0)
    ...
    def forward(self, input):
        ...
        output_ = (input_unfold * alpha).sum(dim=-1)                                           #(nbatch, 4, nx, ny, ninterp)
        #Unnormalize output data
        output = torch.zeros_like(output_)
        output[:, 0, :, :] = output_[:, 0, :, :]
        output[:, 1, :, :] = output_[:, 1, :, :]*output_[:, 0, :, :]
        output[:, 2, :, :] = output_[:, 2, :, :]*output_[:, 0, :, :]
        output[:, 3, :, :] = output_[:, 3, :, :]*output_[:, 0, :, :]
        return output
#---------------------------------------------------------------------------------------------------------------------------------------------
...
def fvm_2ndorder_space_cnn_nolimiter(q0, vert_3d, cell_center, cell_area, dt, r, nhalo, cnn):
    """
    CNN for shallow water equation without flux limiter
    q0: double[4, M+nhalo*2, N+nhalo*2]
    vert_3d: double[3, M+1, N+1]
    cell_center: double[3, M, N]
    cell_area: double[M, N]
    ...
    output = cnn(q0[:, nhalo:-nhalo, nhalo:-nhalo].float().unsqueeze(0))
    output = output.squeeze(0)
    output = torch.moveaxis(output, -1, 0) # [4, M, N，4] -> [4, 4, M, N]
    ...
    output = boundary_condition(output, nhalo, unsqueeze=False)

    #q0l 0 q0u 1 q0r 2 q0b 3
    qcl = output[0, :, nhalo:-nhalo, nhalo:-nhalo]
    qlr = output[2, :, nhalo-1:-nhalo-1, nhalo:-nhalo]

    qcu= output[1, :, nhalo:-nhalo, nhalo:-nhalo]
    qub= output[3, :, nhalo:-nhalo, nhalo+1:-nhalo+1]

    qcr = output[2, :, nhalo:-nhalo, nhalo:-nhalo]
    qrl = output[0, :, nhalo+1:-nhalo+1, nhalo:-nhalo]

    qcb = output[3, :, nhalo:-nhalo, nhalo:-nhalo]
    qbu = output[1, :, nhalo:-nhalo, nhalo-1:-nhalo-1]

    flux_l_high = flux(qcl, qlr, vertbl, vertul, r)
    flux_u_high = flux(qcu, qub, vertul, vertur, r)
    flux_r_high = flux(qcr, qrl, vertur, vertbr, r)
    flux_b_high = flux(qcb, qbu, vertbr, vertbl, r)
...
def fvm_2ndorder_space_cnn(q0, vert_3d, cell_center, cell_area, dt, r, nhalo, cnn):
    """
    CNN for shallow water equation with flux limiter
    q0: double[4, M+nhalo*2, N+nhalo*2]
    vert_3d: double[3, M+1, N+1]
    cell_center: double[3, M, N]
    cell_area: double[M, N]
...
if __name__=="__main__":
    ...
    path = f"{os.path.dirname(os.path.realpath(__file__))}/../data/{args.order}order/torch/{Te}/{args.flux}"
    ...
    vert_3d = vert_3d.cpu()
    MeshGrid.save_sphere_grid_xdmf(f"{path}/sphere-fvm-{M}-{N}-{args.flux}-{args.solver}-torch.xdmf", zip(save_ts, q_out), vert_3d)

...
def compute_flux(q0, vert_3d, r, nhalo, scale):
    ...
    flux_b_scale = flux_b[:, 0::scale, :]
    flux_b_scale = flux_b_scale.reshape(4, M//scale, N//scale, scale)
    flux_b_scale = flux_b_scale.sum(dim=-1)

    flux_l_scale = flux_l[:, :, 0::scale]
    flux_l_scale = flux_l_scale.moveaxis(1, -1)
    flux_l_scale = flux_l_scale.reshape(4,  N//scale, M//scale, scale)
    flux_l_scale = flux_l_scale.sum(dim=-1)
    flux_l_scale = flux_l_scale.moveaxis(-1, 1)

    return torch.stack((flux_b_scale, flux_l_scale), axis=-1)
...
if __name__ == "__main__":
    ...
    with tqdm(range(len(xdmf_data_set)), unit="MB") as pbar:
        for i in pbar:
            ...
            Data_train[i*num_steps:(i+1)*num_steps, ...] = regridder_center(data)
            Data_true[i*num_steps:(i+1)*num_steps, ...] = create_dataset(data, vert_3d, r, nhalo, scale)

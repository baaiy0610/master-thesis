...
def expand_boundary(data_center, data_hori, data_vert):
    ...
    #convert to h, u, v, w
    # data_center[:, 1:, :, :] /= np.expand_dims(data_center[:, 0, :, :], axis=1)
    # data_true[:, 1:, :, :, :] /= np.expand_dims(data_true[:, 0, :, :, :], axis=1)
...
if __name__ == "__main__":
    ...
    x_scale = np.linspace(*x_range, M//scale+1)
    y_scale = np.linspace(*y_range, N//scale+1)
    vert_scale = np.stack(np.meshgrid(x_scale, y_scale, indexing="ij"), axis=-1)
    ...
    #up boundary
    vert_scale_vert_out = 0.5*(vert_scale[0:-1, :] + vert_scale[1:, :])
    lng_vert_out, lat_vert_out = vert_lonlat(vert_scale_vert_out)
    grid_vert_out = {"lon": lng_vert_out.T, "lat": lat_vert_out.T}
    #right boundary
    vert_scale_hori_out = 0.5*(vert_scale[:, 0:-1] + vert_scale[:, 1:])
    lng_hori_out, lat_hori_out = vert_lonlat(vert_scale_hori_out)
    ...
    # regridder_vert = xe.Regridder(grid_in_boundary, grid_vert_out, "nearest_d2s", periodic=True)  #bilinear, conservative, patch, nearest_s2d, nearest_d2s
    ...
    Data_train = zarr.open('data_bilinear_2d/Data_Train.zarr', mode='w', shape=(len(xdmf_data_set)*num_steps, 4, M//scale, N//scale), chunks=(1, 100, M//scale, N//scale), dtype=float)
    Data_true = zarr.open('data_bilinear_2d/Data_True.zarr', mode='w', shape=(len(xdmf_data_set)*num_steps, 4, M//scale, N//scale, 4), chunks=(1, 400, M//scale, N//scale, 4), dtype=float)
